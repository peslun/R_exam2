...
coord_max_list <- df_lst_angle %>% 
  ...
  map(. %>% slice_min(value)) %>% # extract only min values of each group
   map(. %>% slice_min(time)) %>%
  map(. %>% ungroup()) %>% # ungroup dfs
  map(. %>% filter(group !=0 & group !=9)) # remove group = 0 and group = 9
...
first, we create a new list of df that add information about time (originally a row index), similarly as we did for df_lst_long. However, for the map function, we need the dfs in wide format (actually, we need the time and torque variable as vectors side by side). 

Then we map over the dataframe (as .x) and coordinates (as .y) to perform a function to each element. This function is another map, that performs a (second) function for each 5 groups in min list and each 4 groups in max list. The second function is looking for maximum/minimum of Torque within the time constaints (time greater than start as stored in co$start and lower than co$end). The which.max/which.min within the second function finds only the max/min torque.
...
#df_lst_long, min_max_list, speed
```{r}
# Create a list of plots for all 16 files.
plots_all <- map(1:length(df_lst_long), function(i) {
...
sapply(plots_all, function(x) plot(x))
dev.off()
```
*THE END :)*
